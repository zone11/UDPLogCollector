    ...
    this.server.on('message', (msg, rinfo) => {
      try {
        ...
        if (parsed.type === 'LoggedADIF' || parsed.type === 'N1MM') {
          ...
          // Write to ADIF file
          if (this.adifHandler && this.adifHandler.filePath) {
            this.adifHandler.append(qsoData);
          }
          
          // Publish to MQTT
          if (this.mqttClient) {
            this.mqttClient.publish(qsoData);
          }
          
          // Upload to Wavelog
          if (this.wavelogClient) {
            console.log('Uploading to Wavelog...');
            this.wavelogClient.upload(qsoData)
              .then(() => {
                console.log('Wavelog upload successful');
              })
              .catch(error => {
                console.error(`Wavelog upload failed: ${error.message}`);
              });
          }
        } else {
