...
class ADIFHandler {
  constructor(filePath = null) {
    ...
    this.optionalFields = new Set(['sota_ref', 'pota_ref', 'wwff_ref', 'gridsquare', 'name', 'sat_name', 'sat_mode', 'prop_mode']);
  ...
  parse(adifString) {
    ...
    if (fields.sat_name) {
      qso.sat_name = fields.sat_name;
    }
    if (fields.sat_mode) {
      qso.sat_mode = fields.sat_mode;
    }
    if (fields.prop_mode) {
      qso.prop_mode = fields.prop_mode;
    }
  ...
  toADIF(qsoData, format = 'file') {
    ...
    // Determine separator and EOR tag based on format
    ...
    if (qsoData.freq !== null && qsoData.freq !== undefined) {
    ...
    // Optional fields
    if (qsoData.gridsquare) {
      adifFields.push(`<GRIDSQUARE:${qsoData.gridsquare.length}>${qsoData.gridsquare}`);
    }
    if (qsoData.name) {
      adifFields.push(`<NAME:${qsoData.name.length}>${qsoData.name}`);
    }
    if (qsoData.sota_ref) {
    ...
    if (qsoData.sat_name) {
      adifFields.push(`<SAT_NAME:${qsoData.sat_name.length}>${qsoData.sat_name}`);
    }
    if (qsoData.sat_mode) {
      adifFields.push(`<SAT_MODE:${qsoData.sat_mode.length}>${qsoData.sat_mode}`);
    }
    if (qsoData.prop_mode) {
      adifFields.push(`<PROP_MODE:${qsoData.prop_mode.length}>${qsoData.prop_mode}`);
    }
